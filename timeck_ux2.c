...
void read_unix_time(int fd)
{
	 ...
	 */
	{
		 ...
		 */
		long ns_to_next_sec = 1000000000L - tv_local2.tv_nsec;
		ns_to_next_sec -= 100000000L;
		usleep(ns_to_next_sec / 1000L);
		for (;;) {
...
main(int argc, char **argv)
{
	...
	for (i = 0; ; i++) {
		if (i % 20 == 0) {
			printf("# local_clock   locked-timeoff-freqoff   hw_board_clock   us_offset   us_ntpadjtime_offset   ppm\n");
